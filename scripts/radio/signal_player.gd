...
@export var bup_wav: AudioStreamWAV
@export var digit_wavs: Array[AudioStreamWAV] = [] # 10 штук: 0..9

@export var level := 0.8
@export var max_push_per_frame := 4096
...
var _playlist: Array[AudioStreamWAV] = []
var _clip_index := 0
...
func _ready() -> void:
	if bup_wav == null:
		push_error("bup_wav не задан")
		return
	if digit_wavs.size() != 10:
		push_error("digit_wavs должен содержать ровно 10 WAV (0..9)")
		return
	if not (stream is AudioStreamGenerator):
		push_error("В поле stream должен быть AudioStreamGenerator")
		return

	_build_playlist_once()

	var gen := stream as AudioStreamGenerator
	gen.mix_rate = bup_wav.mix_rate
...
func _process(_delta: float) -> void:
	...
	if n > max_push_per_frame:
		n = max_push_per_frame

	for i in range(n):
		var frame := _read_frame()
		var sr := float(_sr) # sample rate текущего клипа (или общий)
		var l := radio_process(frame.x, sr, true)
		var r := radio_process(frame.y, sr, false)
		pb.push_frame(Vector2(l, r) * level)

signal sequence_generated(seq: Array[int])
var _digits_order: Array[int] = []


# ---------- playlist ----------

func _build_playlist_once() -> void:
	_playlist.clear()
	_playlist.append(bup_wav)

	# 0..9 -> shuffle -> берём 7 цифр (без повторов)
	var pool: Array[int] = []
	for i in range(10):
		pool.append(i)
	pool.shuffle()

	var digits: Array[int] = pool.slice(0, 7)

	# строка для сравнения (7 символов)
	var code := ""
	for d in digits:
		code += str(d)
	GameController.set_radio_code(code)

	# плейлист: bup + 7 цифр
	for d in digits:
		_playlist.append(digit_wavs[d])

	_clip_index = 0

func _next_clip() -> void:
	_clip_index += 1
	if _clip_index >= _playlist.size():
		_clip_index = 0 # <-- просто повторяем тот же порядок
	_load_clip(_playlist[_clip_index])

func _load_clip(wav: AudioStreamWAV) -> void:
	_sr = wav.mix_rate
	_stereo = wav.stereo
	_fmt = wav.format
	_data = wav.data
...
func _read_frame() -> Vector2:
	...
	match _fmt:
		...
		_:
			return Vector2.ZERO # IMA ADPCM не поддерживаем

func _is_end() -> bool:
	var ch := 2 if _stereo else 1
	var bytes_per_sample := 2 if _fmt == AudioStreamWAV.FORMAT_16_BITS else 1
	var bytes_per_frame := bytes_per_sample * ch
	var byte_index := _pos_samples * bytes_per_frame
	return byte_index + bytes_per_frame > _data.size()

func _read_frame_16() -> Vector2:
	var ch := 2 if _stereo else 1
	var bytes_per_frame := 2 * ch
	var byte_index := _pos_samples * bytes_per_frame
